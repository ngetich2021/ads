  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        {/* filter bar */}
        <div className="flex gap-3 items-center bg-white rounded-2xl border border-gray-200 shadow-sm px-5 py-4">
        ...
        {[0, 1, 2].map((i) => (
          <div key={i} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="hidden sm:grid grid-cols-[2.5rem_1fr_8rem_10rem_7rem_6rem] gap-2 bg-indigo-50 px-5 py-2.5 border-b border-indigo-100">
              {[...Array(6)].map((_, j) => (
                <div key={j} className="h-3 rounded bg-indigo-100 animate-pulse" />
              ))}
            </div>
            {[0, 1, 2].map((j) => (
              <div
                key={j}
                className="grid grid-cols-[2.5rem_1fr_8rem_10rem_7rem_6rem] gap-2 items-center bg-linear-to-r from-violet-700 to-indigo-700 px-5 py-3.5 border-b border-violet-600/30 last:border-b-0"
              >
